...
FriendlyChat.prototype.loadMessages2 = function() {
  ...
  this.messages2Ref = this.database.ref('messages2');
  // Make sure we remove all previous listeners.
  ...
  var setMessage = function(data) {
    var val = data.val();
    this.displayMessage2(data.key, val.name, val.text, val.photoUrl, val.imageUrl);
  }.bind(this);
  this.messages2Ref.limitToLast(12).on('child_added', setMessage);
  this.messages2Ref.limitToLast(12).on('child_changed', setMessage);
