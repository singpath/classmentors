...
describe('services', function() {
  ...
  describe('clmServices', function() {
    ...
    describe('#canRefresh', function() {
      let profile, codeCombatTimer, freeCodeCampTimer;

      beforeEach(function() {
        profile = {
          $id: 'bob',
          services: {
            codeCombat: {details: {id: 'codecombat:bob'}},
            freeCodeCamp: {details: {id: 'freecodecamp:bob'}}
          }
        };
        ...
        clmServices.registeredWith.withArgs(profile).returns([clmServices.codeCombat, clmServices.freeCodeCamp]);

        sinon.stub(clmServices.codeCombat, 'canRequestUpdate');
        sinon.stub(clmServices.freeCodeCamp, 'canRequestUpdate');
        sinon.stub(clmServices.pivotalExpert, 'canRequestUpdate');

        // by default all timeout value hang.
        codeCombatTimer = {then: sinon.stub()};
        freeCodeCampTimer = {then: sinon.stub()};

        clmServices.codeCombat.canRequestUpdate.returns({timeout: codeCombatTimer});
        clmServices.freeCodeCamp.canRequestUpdate.returns({timeout: freeCodeCampTimer});
        clmServices.pivotalExpert.canRequestUpdate.returns({timeout: {then: sinon.spy()}});
      });

      it('should resolve when any service the user registered with can be refreshed', function() {
        codeCombatTimer.then.yields();

        return clmServices.canRefresh(profile).then(() => {
          expect(clmServices.codeCombat.canRequestUpdate).to.have.been.calledOnce();
          expect(clmServices.freeCodeCamp.canRequestUpdate).to.have.been.calledOnce();
          expect(clmServices.pivotalExpert.canRequestUpdate).to.not.have.been.called();

          expect(clmServices.codeCombat.canRequestUpdate).to.have.been.calledWith(profile.services.codeCombat);
          expect(clmServices.freeCodeCamp.canRequestUpdate).to.have.been.calledWith(profile.services.freeCodeCamp);

          expect(codeCombatTimer.then).to.have.been.calledOnce();
          expect(freeCodeCampTimer.then).to.have.been.calledOnce();
        });
      });

      it('should cancel all used timers before resolving', function() {
        codeCombatTimer.then.yields();

        return clmServices.canRefresh(profile).then(() => {
          expect($timeout.cancel).to.have.been.calledTwice();
          expect($timeout.cancel).to.have.been.calledWith(codeCombatTimer);
          expect($timeout.cancel).to.have.been.calledWith(freeCodeCampTimer);
        });
      });

      it('should resolve early if there are no service to check', function() {
        clmServices.registeredWith.returns([]);

        return clmServices.canRefresh({}).then(() => {
          expect(clmServices.codeCombat.canRequestUpdate).to.not.have.been.called();
          expect(clmServices.freeCodeCamp.canRequestUpdate).to.not.have.been.called();
          expect(clmServices.pivotalExpert.canRequestUpdate).to.not.have.been.called();
        });
      });

      it('should cancel all used timers on error', function() {
        const err = new Error('timeout failed');

        codeCombatTimer.then.throws(err);
    ...
    describe('services', function() {
      ...
      describe('#now', function() {
        ...
        it('should return the current timestamp', function() {
          ...
          expect(service.now() - now).to.be.at.most(10);
        });

      });

      describe('#canRequestUpdate', function() {
        const minute = 60000;
        let timeout;

        beforeEach(function() {
          sinon.stub(service, 'now');
          timeout = Promise.resolve();
          $timeout.returns(timeout);
        });

        it('should throw if no service data are provided', function() {
          expect(() => service.canRequestUpdate()).to.throw();
        });

        it('should return a true ish value if the service has no lastRequest property', function() {
          const canRefresh = service.canRequestUpdate({});

          expect(canRefresh.value).to.be.true();
        });

        it('should return a true ish value if the lastRequest is older than a minute', function() {
          const lastUpdateRequest = 1;

          service.now.returns(lastUpdateRequest + minute);

          const canRefresh = service.canRequestUpdate({lastUpdateRequest});

          expect(canRefresh.value).to.be.true();
        });

        it('should return a false ish value if the lastRequest is less than a minute old', function() {
          const lastUpdateRequest = 1;

          service.now.returns(lastUpdateRequest + minute - 1);

          const canRefresh = service.canRequestUpdate({lastUpdateRequest});

          expect(canRefresh.value).to.be.false();
        });

        it('should return a mocked timeout promise if the user can refresh', function() {
          const lastUpdateRequest = 1;
          ...
          expect(canRefresh.timeout).to.not.equal(timeout);
          expect(canRefresh.cancel).to.be.a('function');

          canRefresh.cancel();
          expect($timeout.cancel).to.not.have.been.called();
        });

        it('should return a cancellable timeout promise when the user cannot yet refresh', function() {
          const lastUpdateRequest = 1;
          ...
          expect(canRefresh.timeout).to.equal(timeout);
          expect(canRefresh.cancel).to.be.a('function');

          canRefresh.cancel();
          expect($timeout.cancel).to.have.been.calledOnce();
          expect($timeout.cancel).to.have.been.calledWith(canRefresh.timeout);
        });
